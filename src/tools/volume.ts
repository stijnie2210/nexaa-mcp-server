...
export function registerVolumeTools(
  ...
  server.registerTool(
    ...
    async ({ namespace }) => {
      ...
      return {
        content: [
          { type: "text", text: JSON.stringify(data.namespace.volumes, null, 2) },
        ],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ namespace, name, size }) => {
      const data = await client.request<{ volumeCreate: unknown }>(
        VOLUME_CREATE,
        { input: { namespace, name, size } }
      );
      return {
        content: [
          { type: "text", text: JSON.stringify(data.volumeCreate, null, 2) },
        ],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ namespace, name, size }) => {
      const data = await client.request<{ volumeIncrease: unknown }>(
        VOLUME_INCREASE,
        { input: { namespace, name, size } }
      );
      return {
        content: [
          { type: "text", text: JSON.stringify(data.volumeIncrease, null, 2) },
        ],
      };
    }
  );
